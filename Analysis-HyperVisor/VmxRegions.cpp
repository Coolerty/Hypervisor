...
AllcoateVmxOn(vcpu* Vpcu)
{
	...
	Vpcu->VMXONRegion = (uint64_t)MmAllocateContiguousMemory(VMXON_SIZE, PhysicalMax);
...
bool AllocateVmcs(vcpu* Vpcu)
{
	...
	Vpcu->VMCSRegion = (uint64_t)MmAllocateContiguousMemory(VMCS_SIZE, PhysicalMax);
...
bool AllocateVmmStack(vcpu* Vpcu)
{
	...
	Vpcu->VMMStack = (uint64_t)ExAllocatePool2(POOL_FLAG_NON_PAGED, VMM_STACK_SIZE, (ULONG)POOL_TAG);
...
bool AllocateMsrBitmap(vcpu * Vpcu)
{
	...
	Vpcu->MsrBitmap = (uint64_t)ExAllocatePool2(POOL_FLAG_NON_PAGED, PAGE_SIZE, (ULONG)POOL_TAG);  // should be aligned
	...
	Vpcu->PhysicalMsrBitmap = (uint64_t)VirtualAddressToPhysicalAddress((PVOID)Vpcu->MsrBitmap);
